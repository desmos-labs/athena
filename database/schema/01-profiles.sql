...
CREATE TABLE default_chain_link
(
    id              SERIAL NOT NULL PRIMARY KEY,
    user_address    TEXT   NOT NULL,
    chain_link_id   BIGINT NOT NULL REFERENCES chain_link (id) ON DELETE CASCADE,
    chain_config_id BIGINT NOT NULL REFERENCES chain_link_chain_config (id),
    height          BIGINT NOT NULL,
    CONSTRAINT unique_default_chain_link UNIQUE (chain_config_id, user_address)
);
CREATE INDEX default_chain_link_user_address_index ON default_chain_link (user_address);
CREATE INDEX default_chain_link_chain_link_id_index ON default_chain_link (chain_link_id);
CREATE INDEX default_chain_link_chain_config_id_index ON default_chain_link (chain_config_id);
